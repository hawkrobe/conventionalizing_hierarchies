...
for(i in c(gameIDs)) {
  ...
  result <- fromJSON(file = paste0("../models/holistic/bdaOutput/sextiles/", i, ".json"))
...
```{r}
RSA.entropies <- read_csv('../models/holistic/bdaOutput/lexicalInferenceSextilePredictives.csv') %>%
...
ggplot(RSA.entropies %>% mutate(quartile = paste0('epoch ', quartile)), aes(x = entropy, fill = condition)) +
  geom_density(alpha = 0.5, adjust = 1/6) +
  facet_wrap( ~ quartile, nrow = 2, ncol = 3) +
  theme_few() +
  scale_color_colorblind() +
  theme(aspect.ratio = 1, legend.position = 'top') 
  #guides(fill = F) 
...
ggsave("../writing/cogsci18/figures/entropies.pdf", height = 5, width = 6)
```
...
RSA.entropies %>% 
  ...
  summarize(m = mean(entropy.diff), se = sd(entropy.diff)/sqrt(length(entropy.diff))) %>%
...
for(i in c(gameIDs)) {
  result <- fromJSON(file = paste0("../models/holistic/bdaOutput/sextiles/", i, ".json"))
  #print(result)
  gameDF = data.frame(speaker_qt1_mu = result$VarMu$data, speaker_qt1_sigma = result$VarSigma$data)
  varianceDF = rbind(varianceDF, gameDF %>% mutate(gameid = i))
}
```

```{r}
varianceDF %>% mutate(sigma = log(1 + exp(speaker_qt1_mu))) %>% ggplot(aes(x = sigma)) + geom_histogram()  + theme_few()
```
