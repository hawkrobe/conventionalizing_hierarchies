...
postTest_clean <- postTest_raw %>%
  ...
  # select(-objectToWordMeaning, -wordToObjectMeaning) %>%
  # spread(finalRole, meaning) %>%
  # mutate(match = listener & speaker) %>%
print(length(unique(postTest_clean$gameid)))
```

### Check whether cleaning out non-covergers basically equates internal consistency
...
# summary(lm(pctConsistent ~ condition, data = postTest_clean %>%
#   group_by(gameid, finalRole, condition) %>%
#   summarize(pctConsistent = 128- sum(internalConsistency))))
```
...
summary(lm(pctAbstract ~ condition, data = pctDF))
  ...
  #gather(type, number, numSpecific, numAbstract) 
  #ungroup() %>%
  # group_by(condition) %>%
  # complete(gameid, finalRole, fill = list(pct = 0)) %>%
  # spread(abstract, pct) %>%
...
Split into 8ths; show average difference in entropy from one 8th to the next...

Might be interested in deviation from 

Take entropy of normalized rows of lexicon; 

Clarify in the paper that we're getting the effective lexicon, not what's in peoples' heads. 

Peoples' usage is surprisingly tight... 

Add a sentence about associations, e.g. 'nogo' ; 
...
One signature of more or less specific words is the size of their extension. We extract this from lexical posteriors by computed the expected entropy on the normalized set of meanings for each word... 

To do this, we first group by participants and words to get the vector of posterior params for each object. We then take a sample from this posterior, feed through a sigmoid function to map to [0,1], normalize these values to sum to 1, and take the entropy. 

We repeat this sampling process many times to get an estimate (mean & sd) of the true entropy for each word for each pair. 

Once we have these we can (1) plot a histogram by condition and quartile, (2) see how well it corresponds to the extensions measured in post-test (In final quartile, the expected entropy should predict the size of the extension... )

This is fairly hacky; below, we show the litener responses from RSA

```{r}
# element-wise sigmoid function
# sigmoid <- function(x) {return(1 / (1 + exp(-x*5)))}
# normalize <- function(x) {return(exp(20*x) / sum(exp(20*x)))}
# entropy <- function(p) {return(sum(-p*log2(p)))}
# computeEntropy <- function(df) {
#   samples <- replicate(10, {
#     random <- rnorm(df$mu, mean=df$mu, sd = .01)
#     transformed <- sigmoid(random)
#     normalized <- normalize(transformed)
#     entropied <- entropy(normalized)
#     return(entropied)
#   })
#   return(df %>% mutate(entropyEstimate = mean(samples)))
# }
# 
# length((posteriors %>%
#   inner_join(postTest_clean) %>%
#   group_by(id, finalRole, quarter, wordID, object, mu, sigma) %>%
#   summarize(condition = first(condition)))$id)
# 
# # Just to get a feel for it, start with just one game and one quarter
# estimatedEntropies <- posteriors %>%
#   left_join(postTest_clean) %>%
#   group_by(id, finalRole, quarter, wordID, object, mu, sigma) %>%
#   summarize(condition = first(condition)) %>%
#   #filter(id == 'game4') %>% filter(quarter == 4) %>% filter(wordID == 'word6') %>%
#   group_by(id, condition, finalRole, quarter, wordID) %>%
#   do(computeEntropy(.)) 
# 
# ggplot(estimatedEntropies %>% ungroup() %>% filter(!(is.na(condition))), aes(x = entropyEstimate)) +
#   geom_density() +
#   facet_grid(condition ~ quarter) + 
#   theme_few()
```

```{r}
RSA.entropies <- read_csv('../models/holistic/bdaOutput/lexicalInferencePredictives.csv') %>%
  mutate(finalRole = ifelse(finalRole == 'finalSpeaker', 'speaker', 'listener')) %>%
  left_join(masterGameIDLookup) #%>%
  # group_by(id, finalRole, quarter, wordID, con) %>%
  # summarize(condition = first(condition), entropy = mean(entropy))

ggplot(RSA.entropies, aes(x = entropy, fill = condition)) +
  geom_density(alpha = 0.5, adjust = 2/3) +
  facet_grid( ~ quarter) +
  theme_few() +
...
RSA.entropies %>% 
  ...
  summarize(m = mean(entropy.diff)) %>%
  ggplot(aes(x = condition, y = m, fill = factor(quarter))) +
    geom_bar(position = 'dodge', stat='identity') +
    scale_fill_manual(values = c("gray34", "gray55", "gray77")) +
    theme_few() +
    ylab("mean difference in entropy from previous round")
ggsave("../writing/cogsci18/figures/entropies.pdf", height = 3, width = 5)
```

see how it corresponds to the size of the extensions?
...
Kinda confused about this... It looks like it's learning the hierarchical thing pretty well but basically loosening the constraints on local rounds late in the game. It looks like lexica learned for local rounds only have the minimal thing necessary to predict that round, but without the global properties. Why would this be better for the likelihood, though? Maybe you just don't want to put strong predictions on stuff you don't observe? Wants each local round to only have the minimal thing required?
